...
public class QAController {
    ...
    public ResponseEntity<List<Map<String, Object>>> search(@RequestBody SearchRequest request) {
        List<Map<String, Object>> results = qaService.searchQuestion(request.getQuestion());
    ...
    public ResponseEntity<String> like(@RequestBody LikeRequest request) {
        qaService.likeQuestion(request.getQuestion(), request.getAnswer());
        return ResponseEntity.ok("Đã lưu câu hỏi và câu trả lời");
    }

    // gửi câu hỏi tư vấn
    @PostMapping("/consult")
    public ResponseEntity<String> consult(@RequestBody ConsultRequest request) {
        qaService.consult(request.getQuestion(), request.getUserEmail());
